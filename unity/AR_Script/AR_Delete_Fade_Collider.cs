...
public class AR_Delete_Fade_Collider : MonoBehaviour
{
    // 메뉴창에 추가
    [MenuItem("Tools/AR/Make Delete Collider", false, 0)]
    private static void AR_Delete_Fade_ColliderCommand(MenuCommand menuCommand)
    {
        // 선택한 오브젝트
    ...
    public static bool delete_Transform(Transform selectedTransform)
    {
        ...
        DeleteDescendantsRecursive(selectedTransform);

        delete_C_ObjFade(selectedTransform.gameObject);
