...
public class AR_Animation_delete_action : MonoBehaviour
{
        ...
        if (selectedTransform != null)
        {
            ...
            foreach (Transform child in childTransforms)
            {
                ...
                if (child != selectedTransform.transform && !child.name.Contains("Rotation"))
                {
                    ...
                    // 월드 위치를 유지하도록 로컬 위치를 조정 (선택사항)
                    child.localPosition = selectedTransform.localPosition;
                    //child.localRotation= selectedTransform.localRotation;
                }
            }
